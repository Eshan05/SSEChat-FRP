...
// 1. Axios Instance with Timeouts, Retries & Connection Pooling
...
// Exponential backoff retry (only for non-streaming paths or initial connection)
...
const ollamaRoutes: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.get('/models', async (_request, reply) => {
    try {
      const response = await axiosInstance.get(`${OLLAMA_BASE}/api/tags`);
  ...
  fastify.get('/models/info', async (request, reply) => {
    ...
    try {
      const response = await axiosInstance.post(`${OLLAMA_BASE}/api/show`, { model });
  ...
  fastify.post('/chat', async (request, reply) => {
    ...
    if (messages.length > 100) {
      return reply.code(413).send({ error: "Too many messages" });
    }
    ...
    const eventSourceFactory = () =>
      (async function* (): AsyncGenerator<SSEEvent> {
        ...
        // 3. Ensure cleanup triggers on all termination paths
        const cleanup = () => {
          upstream?.destroy();
          upstreamAbort.abort();
        };
        ...
        clientSocket.on('finish', cleanup);
        ...
        try {
          ...
          upstream.on('error', cleanup); // Handle upstream errors
          ...
          if ((error as Error).name !== 'AbortError') {
            ...
            // 8. Standardize upstream-to-client error message contract
            yield { event: 'error', data: JSON.stringify({ error: formattedError }) } satisfies SSEEvent;
          }
        } finally {
          clientSocket.off('close', cleanup);
          clientSocket.off('finish', cleanup);
...
async function sendEventStream(
  ...
  for await (const event of sourceFactory()) {
    ...
    const eventName = typeof event !== 'string' && event.event ? `event: ${event.event}\n` : '';

    // 2. Backpressure + stream error propagation
    const canWrite = reply.raw.write(`${eventName}data: ${payload}\n\n`);
    if (!canWrite) {
